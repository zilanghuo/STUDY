...
@Slf4j
@Service
public class Dh3tSendNotifyService extends AbstractDh3tSmService implements ISendSmService, InitializingBean {
    //发送短信线程池
    private ExecutorService executor;

    @Override
    public MessageResultDto<SendSmCommonRspDto> sendSm(SendSmCommonReqDto sendSmCommonReqDto) {
        dh3tSendMessage(SmChannelConfig.DH3T_MSG, sendSmCommonReqDto, SendSmType.NOTIFY, executor);
    ...
    public void afterPropertiesSet() throws Exception {
        executor = new ThreadPoolExecutor(50, 500, 100L, TimeUnit.MINUTES, new LinkedBlockingQueue<Runnable>(),
                new SmThirdProviderThreadFactory("Dh3tSendNotifyService-pool-"));
    }
}
