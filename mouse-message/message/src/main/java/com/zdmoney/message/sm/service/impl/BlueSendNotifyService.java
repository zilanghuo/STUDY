 ...
 */
@Slf4j
@Service(value = "blueSendNotifyService")
public class BlueSendNotifyService extends AbstractBlueSmService implements ISendSmService, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        executor = new ThreadPoolExecutor(50, 500, 100L, TimeUnit.MINUTES, new LinkedBlockingQueue<Runnable>(),
                new SmThirdProviderThreadFactory("BlueSendNotifyService-pool-"));
    }

    @Override
    public MessageResultDto<SendSmCommonRspDto> sendSm(SendSmCommonReqDto sendSmCommonReqDto) {
        blueSendMessage(SmChannelConfig.BLUE_MSG, sendSmCommonReqDto, SendSmType.NOTIFY, executor);
