 ...
 */
public class TraceHessianSkeleton extends HessianSkeleton {

    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(TraceHessianSkeleton.class);
     ...
     */
    public TraceHessianSkeleton(Object service, Class<?> apiClass) {
        super(apiClass);
     ...
     * @param apiClass the API interface
     */
    public TraceHessianSkeleton(Class<?> apiClass) {
        super(apiClass);
    }
    ...
    public void invoke(Object service,
        ...
        try {
            ...
            logger.info("HessianServer receive traceId[{}] clz[{}] method[{}] arguments[{}] ",
                    new Object[]{MDC.get(Constant.TRACE_ID), _apiClass.getSimpleName(), receiveMethodName, JsonUtils.toJson(values)});
            result = method.invoke(service, values);
            long endTime = new Date().getTime();
            long time = endTime - startTime;
            if (!StringUtils.contains(receiveMethodName, "search")) {
                logger.info("HessianServer response traceId[{}] clz[{}] method[{}] arguments[{}] time[{}]",
                        new Object[]{MDC.get(Constant.TRACE_ID), _apiClass.getSimpleName(), receiveMethodName, JsonUtils.toJson(result), time});
            } else {
                logger.info("HessianServer response traceId[{}] clz[{}] method[{}] time[{}]",
                        new Object[]{MDC.get(Constant.TRACE_ID), _apiClass.getSimpleName(), receiveMethodName, time});
            }
            ...
            logger.info(this + " " + e1.toString() + e1);
    ...
    protected PrintWriter createDebugPrintWriter()
            throws IOException {
        return new PrintWriter(new LogWriter(logger));
    }
